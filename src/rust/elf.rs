...
use anyhow::{Context};
use goblin::elf::{Elf, Header, ProgramHeader, Sym, program_header::PT_DYNAMIC, program_header::PT_LOAD, reloc::R_X86_64_JUMP_SLOT, Reloc};
use memmap2::Mmap;
use std::fs::File;
use std::ops::Deref;
...
impl ExecuteLinkFile {
    ...
    #[allow(unused)]
    pub fn get_elf_header(&self) -> Result<Header, Box<dyn std::error::Error>> 
    {
        let h = self.borrow_elf().header;
        Ok(h)
    }
    ...
    #[allow(unused)]
    pub fn prase_sym(&self, name: &str) -> Result<Sym, Box<dyn std::error::Error>> {
        let sym = self.borrow_elf()
            .syms.iter()
            .find(|sym| self.get_str(sym.st_name).ok().as_deref() == Some(name))
            .context(format!("No symbol found with name {}", name))?;
    ...
    pub fn get_str(&self, location: usize) -> Result<String, Box<dyn std::error::Error>> {
        let str = self.borrow_elf()
            .strtab
            .get_at(location)
            .context(format!("Could not get dynstr at location {}", location))?;
